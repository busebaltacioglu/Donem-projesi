...
##Veri seti kayıp değer kontrolü
```{r}
library(funModeling)
...
###Kayıp gözlem oranı hesaplama
```{r}
kdy<-function(x) {sum(is.na(x))/length(x)*100}
apply(data1, 2, kdy)
```
-Örneğin, glucose'un %9 u kayıp değerdir.
...
#Mice paketi ile kayıp değer atama random forest
```{r}
set.seed(2882)
...
summary(Ct1)
```
Terminal node sayısı 4, artıkların ortalamadan sapması yaklaşık 0.79 ve accuracy=0.75
...
```
Grafiğe baktığımızda 2-5 denenmeli diyebiliriz.

```{r}
prune_ctree1<-prune.tree(Ct1, best=5)
summary(prune_ctree1)
```
Terminal node sayısı 3, artıkların ortalamadan sapması %81 ve accuracy=75
...
```{r}
attach(trainn)
lr1<-glm(TenYearCHD~age+cigsPerDay+totChol+sysBP+glucose+sex+prevalentStroke, data = trainn, family = "binomial")
detach(trainn)
summary(lr1)
```
...
```{r}
lr1_pred_train<-ifelse(lr1_pred_train<=0.49,0,1)
lr1_pred_test<-ifelse(lr1_pred_test<=0.49,0,1)
...
```{r}
costs <- seq(from=0.05, to=20, by=0.05)
...
```{r}
xgb1_pred_train<-ifelse(xgb1_pred_train<=0.525,0,1)
xgb1_pred_test<-ifelse(xgb1_pred_test<=0.525,0,1)
...
```{r}
set.seed(2882)
knn1<-knn(train = train.x1, test = test.x1, cl = trainn[,15], k = 88)
```
